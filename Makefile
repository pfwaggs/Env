...
ifeq (.git,$(notdir $(findstring .git,$(shell ls -d .git 2>/dev/null))))
  ifeq (,$(filter list info status help filelist long short envcheck,$(MAKECMDGOALS)))
...
envcheck :
	@for x in $(ENVCHECK); do
	    [[ $$(compgen -A function) =~ $$x ]] || { echo ENVCHECK failure; exit 1; }
	done
...
%-xinfo %-xrecover %-xtar %-xtargz : 
	@ref=$$(perl -n -E '$*==$$. && say /-(\w+)$$/' archive.txt)
	cmd=$@
	cmd="$${cmd#*x}"
	make $$ref-$$cmd
...
%-recover :
	@x=$$(grep $* archive.txt)
	[[ -n $$x ]] || { echo $* not found in archive.txt; exit 1; }
...
filelist :
	...
	cd $(ENVDIR) &>/dev/null
	load fsplit
	fsplit prime.rc extra.rc 2>/dev/null
	echo $(MAKEFILE) > $@
	find $(ENVDIR)/{,dotinstall,support} -maxdepth 2 -type f | \
	    grep -v '\.rc' | sort >> $@
	echo removing old print files
	rm -r long* short* md5sums* 2>/dev/null
